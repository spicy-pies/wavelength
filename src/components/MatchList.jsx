...
function calcCompatibility(a, b) {
    // semantic similarity if both have vectors
    if (a.vector && b.vector) {
        const score = Math.round(cosineSimilarity(a.vector, b.vector) * 100)
        
        // shared tags for display only
        const allKeys = ["interests", "music", "shows"]
        let shared = []
        allKeys.forEach(key => {
        const aSet = new Set(a[key] || [])
        ;(b[key] || []).forEach(x => { if (aSet.has(x)) shared.push(x) })
        })

        return { score, shared }
    }

    // fallback to tag overlap if no vectors yet
    const allKeys = ["interests", "music", "shows"]
    let shared = []
    let total = new Set()
    allKeys.forEach(key => {
...
function MatchCard({ match, user, room }) {
    ...
    useEffect(() => {
        ...
        (snap) => {
            ...
            .map(d => d.data())
            .filter(m => m.uid !== user.uid && m.visible && calcCompatibility(profile, m).score >= 10)
